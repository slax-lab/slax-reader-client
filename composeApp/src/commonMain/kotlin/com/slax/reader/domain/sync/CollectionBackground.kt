...
class CollectionBackgroundDomain(
    ...
    suspend fun startup() = lifecycleMutex.withLock {
        ...
        scope.launch {
            queue.receiveAsFlow()
                ...
                .collect {
                    AppLog.d("[CollectionBackgroundDomain] task completed")
                }
        }
    }
    ...
    private suspend fun downloadBookmark(task: CollectionTaskItem) {
        ...
        try {
                    ...
                    launch {
                        content.imageUrls.forEach { url ->
                            try {
                                ...
                                AppLog.d("[CollectionBackgroundDomain] image cache failed: ${error.message}")
                            }
                        }
                    }
                }
            ...
            AppLog.d("[CollectionBackgroundDomain] download failed ${task.bookmarkId}: ${error.message}")
            failedInSession.getAndUpdate { it + task.cacheKey }
            try {
                ...
                AppLog.d("[CollectionBackgroundDomain] failed to persist FAILED status: ${statusError.message}")
            }
        } finally {
        ...
        try {
            ...
            ids.forEach { id ->
                val isManual = try {
                    ...
                    AppLog.d("[CollectionBackgroundDomain] failed to verify cache ownership $id: ${error.message}")
                    true
                }
                if (
                    id in inQueue.value ||
                ...
                try {
                    if (fileManager.deleteDataDirectory("bookmark/$id")) {
                        ...
                        AppLog.d("[CollectionBackgroundDomain] cleanup failed $id")
                    }
                } catch (error: CancellationException) {
                    throw error
                } catch (error: Exception) {
                    AppLog.d("[CollectionBackgroundDomain] cleanup failed $id: ${error.message}")
                }
            }
            try {
                ...
                AppLog.d("[CollectionBackgroundDomain] failed to delete cache state: ${error.message}")
            }
        } finally {
    ...
    suspend fun getBookmarkContent(id: String, ownerId: String): ProcessedContent {
        ...
        try {
            ...
            val content = try {
                ...
                AppLog.d("[CollectionBackgroundDomain] API request failed $id: ${error.message}")
                failedInSession.getAndUpdate { it + cacheKey }
            ...
            try {
                ...
                AppLog.d("[CollectionBackgroundDomain] cache write failed $id: ${error.message}")
                failedInSession.getAndUpdate { it + cacheKey }
                updateForegroundStatus(cacheKey, DownloadStatus.FAILED)
            }
            return content
    ...
    private suspend fun updateForegroundStatus(cacheKey: String, status: DownloadStatus) {
        try {
            ...
            AppLog.d("[CollectionBackgroundDomain] failed to persist $status status: ${error.message}")
        }
    }
    ...
    private fun processContent(html: String): ProcessedContent {
        return runCatching {
            ...
            AppLog.d("[CollectionBackgroundDomain] HTML processing failed: ${error.message}")
