...
fun MarkdownRenderer(
    ...
    val content = outlineState.outline
    val preprocessedContent = preprocessMarkdownLinks(content, outlineState.isLoading)
 ...
 * 预处理 Markdown 链接
 */
private fun preprocessMarkdownLinks(content: String, isLoading: Boolean): String {
    var result = content

    // 处理带空格的完整链接：[text](#url with spaces) -> [text](<#url with spaces>)
    val linkPattern = Regex("""\[([^\]]+)\]\(([^)<>\s]+(?:\s+[^)<>]+)+)\)""")
    result = linkPattern.replace(result) { matchResult ->
    ...
    // 移除字符串末尾的不完整锚点，匹配所有 [文本](#任意内容 直到字符串末尾的模式
    val possibleAnchorPattern = Regex("""\[([^\]]+)\]\(#.*$""")
    val match = possibleAnchorPattern.find(result)

    // 如果找到匹配且确认是不完整的锚点，则移除
    if (match != null && isIncompleteAnchor(match.value)) {
        result = result.removeRange(match.range)
    }

    // 步骤3：处理流式传输时可能在末尾出现不完整的列表项标记
    if (result.endsWith("-")) {
...
/**
 * 判断是否为不完整的锚点
 * 通过检查括号平衡来判断：如果左括号多于右括号，说明锚点未闭合
 *
 * @param text 匹配到的锚点文本，例如：[1](#文本(UGA)更多内容
 * @return true 表示不完整，false 表示完整
 */
private fun isIncompleteAnchor(text: String): Boolean {
    // 找到 (# 的位置
        ...
        Column(modifier = Modifier.weight(1f)) {
            node.children.forEach { child ->
                ...
                when {
                    ...
                    // Setext 标题（在列表中被误识别的情况）
                    childType == "Markdown:SETEXT_1" || childType == "Markdown:SETEXT_2" -> {
                        RenderHeading(
                            content = content,
                            node = child,
                            fontSize = 15.sp,
...
private fun RenderHeading(
    ...
    // 查找标题内容节点（ATX_CONTENT 用于 # 标题，SETEXT_CONTENT 用于 Setext 标题）
    val contentNode = node.children.firstOrNull {
        val type = it.type.toString()
        type == "Markdown:ATX_CONTENT" || type == "Markdown:SETEXT_CONTENT"
    }

    // 对于 Setext 标题，直接获取文本并移除开头的 # 符号
    if (contentNode != null && contentNode.type.toString() == "Markdown:SETEXT_CONTENT") {
        val headingText = contentNode.getTextInNode(content).toString()
    ...
    val contentNodes = if (contentNode != null) {
        ...
        node.children.filter {
            val type = it.type.toString()
            // 过滤掉空白节点、EOL 和 Setext 标记节点
            !type.contains("WHITE_SPACE") &&
            type != "Markdown:EOL" &&
            type != "Markdown:SETEXT_1" &&
            type != "Markdown:SETEXT_2"
        }
    }
