...
actual fun UserWebView(
    ...
    val scriptMessageHandler = remember {
        object : NSObject(), WKScriptMessageHandlerProtocol {
            override fun userContentController(
                ...
                val body = didReceiveScriptMessage.body as? String
                if (body != null) {
                    runCatching { Json.decodeFromString<WebViewMessage>(body) }
                        .onSuccess { msg ->
                            when (msg.type) {
                                "getSubscription" -> {
                                    userWebViewState.dispatchEvent(WebViewEvent.GetSubscription)
                                }
                                "startSubscription" -> {
                                    userWebViewState.dispatchEvent(WebViewEvent.StartSubscription)
                                }
                            }
                        }
                }
            ...
            if (cookie != null && cookieStore != null) {
                kotlinx.coroutines.suspendCancellableCoroutine { continuation ->
                    cookieStore.setCookie(cookie) {
                        println("[iOS UserWebView] Cookie已注入: Domain=${SlaxConfig.WEB_DOMAIN}")
    ...
    LaunchedEffect(userWebViewState) {
        userWebViewState.commands.collect { cmd ->
            when (cmd) {
                is WebViewCommand.EvaluateJs -> {
                    userWebViewState.webView?.evaluateJavaScript(cmd.script) { result, error ->
                        if (error != null) {
                            println("[iOS UserWebView] JS 执行失败: ${error.localizedDescription}")
                        }
                        cmd.callback?.invoke(result?.toString() ?: "")
                    }
                }
    ...
    DisposableEffect(Unit) {
        onDispose { userWebViewState.webView = null }
    }
}
